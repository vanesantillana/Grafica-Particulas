...
void initParticles(void){
	...
	for(i=0; i<currentParticles; i++){			//for each particle we set random variables for all parameters
		Particles[i].XCoor = rand()%800*1.0;
		Particles[i].YCoor = rand()%800*1.0;
...
void display(void){
	if(pause == 0){
		...
		string str3("q-e = mayor/menor velocidad");
		...
		string str5("z-c grandes/Chiquitas");
	...
	for(i=0;i<currentParticles;i++){
		...
		glColor3f(Particles[i].Red,Particles[i].Red,Particles[i].Red);
